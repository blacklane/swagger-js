...
Operation.prototype.getHeaderParams = function (args) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if (typeof args[param.name] !== 'undefined' && args[param.name].length) {
      if (param.in === 'header') {
        var paramValue = args[param.name].shift();
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if (typeof args[param.name] !== 'undefined' && args[param.name].length) {
      if (param.in === 'path') {
        ...
        var value = args[param.name].shift();
        ...
        if (typeof param.collectionFormat !== 'undefined') {
          var qp = args[param.name].shift();

          if (Array.isArray(qp)) {
            querystring += this.encodeQueryCollection(param.collectionFormat, param.name, qp);
          } else {
            querystring += this.encodeQueryParam(param.name) + '=' + this.encodeQueryParam(qp);
          }
        } else {
          querystring += param.name + '=' + this.encodeQueryParam(args[param.name].shift());
        }
      } else if (param.in === 'formData') {
        formParams[param.name] = args[param.name].shift();
      }
    }
  }
...
Operation.prototype.getBody = function (headers, args, opts) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if (typeof args[param.name] !== 'undefined' && args[param.name].length) {
      if (param.in === 'body') {
        body = args[param.name].shift();
      } else if (param.in === 'formData') {
        if(formParams[param.name] === undefined) { formParams[param.name] = []; }
        formParams[param.name].push(args[param.name].shift());
      }
    }
  }
  ...
  if (headers['Content-Type'] === 'application/x-www-form-urlencoded') {
    ...
    for(var i = 0; i < this.parameters.length; i++) {
      ...
      if(param.in === 'formData' && typeof formParams[param.name] !== 'undefined' && formParams[param.name].length) {
        var paramValue = formParams[param.name].shift();

        if(encoded !== '') { encoded += '&'; }
        encoded += param.name + '=' + encodeURIComponent(paramValue);
      }
    }
